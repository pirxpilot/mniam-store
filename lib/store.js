...
const debug = Debug('connect:mniam-store');
...
function serializeSession(session) {
  const { cookie, ...obj } = session;
  ['expires', 'originalMaxAge'].forEach(prop => {
    if (prop in cookie) {
      obj.cookie = {};
      obj.cookie[prop] = cookie[prop];
    }
  });
  return obj;
}
...
export default function store(middleware) {
  ...
  class MniamStore extends Store {
    ...
    get(sid, fn) {
      debug('Get "%s"', sid);
      this.sessions.findOne({ _id: sid }).then(data => {
        if (!data) {
          return fn();
        }
    ...
    set(sid, session, fn) {
      ...
      this.sessions.findOneAndUpdate({ _id: sid }, update, { upsert: true }).then(() => fn(), fn);
    }

    touch(sid, _session, fn) {
      const update = {
        $currentDate: {
          _mod: true
        }
      ...
      this.sessions.findOneAndUpdate({ _id: sid }, update).then(() => fn(), fn);
    }
    ...
    destroy(sid, fn) {
      debug('Destroy "%s"', sid);
      this.sessions.deleteOne({ _id: sid }).then(() => fn(), fn);
    }
  }
